...
fun OrderDetailScreen(
    ...
    // Function to check if a potential new total would exceed tendered amount
    ...
    fun handleItemUpdate(menuItem: MenuItem, quantity: Int, editIdx: Int) {
        val itemCost = if (editIdx >= 0) {
            // For editing, calculate the difference in cost
            menuItem.price * quantity - items[editIdx].getTotalPrice()
        } else {
            // For new items, calculate the full cost
            menuItem.price * quantity
        }

        if (wouldExceedTendered(itemCost)) {
            // Store pending values and show warning
    ...
    // Function to apply pending item update after tendered amount is updated
            ...
            TopAppBar(
                ...
                actions = {
                    if (!isEditing) {
                        ...
                        IconButton(onClick = {
                            // Validate tendered amount before saving
                            if (isValidTendered) {
                    ...
                    items.forEachIndexed { idx, item ->
                        Row(
                            ...
                            if (isEditing) {
                                IconButton(onClick = {
                                    editIndex = idx
                                    selectedMenuItemForQuantity = availableMenuItems.find { it.name == item.name }
                    ...
                    TextButton(
                        onClick = {
                            ...
                            if (newTendered != null && newTendered >= newTotal) {
                                // Update the order with new tendered amount
                                val updatedOrder = orderRecord.copy(
                                    amountTendered = newTendered
                                )
                                onUpdateOrder(updatedOrder)
                                // Apply the pending item update
                                applyPendingUpdate()
